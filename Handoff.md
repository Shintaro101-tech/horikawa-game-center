...
- **時代（編）構造**: 5編 × 各18ステージ予定（`ERA_DATA`）
  ...
  - 現状 **過去編 + 超過去編 + 現代編が available**、未来/超未来は COMING SOON
  - 現代編は **超過去編 全18ステージクリア** で解放
...
- **18ステージ × 3編 = 54ステージ実装済**（過去 1-18 / 超過去 101-118 / 現代 201-218）+ ステージ別解放（`STAGE_UNLOCKS`）
...
- **現代編 全18ステージを実装**（id:201-218）— 超過去編クリアで解放
  ...
  - **18体のボス**（`STAGE_BOSSES[201..218]`）:
    ...
    17 暗黒大企業CEOザイバツ(召喚) / 18 サタライト・ゼロ(メガ・軌道レーザー)
