...
- **アーケードに戻る**: タイトル画面(`#title`)に `<a class="btn ghost" href="../">🏠 アーケードにもどる</a>`。さらに**プレイ中HUDの右上**にも `🏠ホーム`ミニボタン(`#btn-home`、図鑑の左 right:72px・金グラデ)を配置＝`save()`してから`location.href='../'`（自動セーブ済なので進行は失われない）。`../`＝ゲームセンターのホーム。レスポンシブ右側位置は ≤400px=right:60 / ≥820px=right:84 で図鑑と重ならないよう調整。
...
### 🏠 2026-06-28 プレイ中HUDにもアーケードへ戻るボタン追加
- HUD右上に `🏠ホーム`ミニボタン(`#btn-home`、図鑑の左)を追加＝`save()`してから`location.href='../'`。自動セーブ済なので進行は失われない。右側位置は ≤400px=right:60 / 既定=right:72 / ≥820px=right:84 で図鑑と非重複。検証: 全幅で重なりなし、クリックで`/`(アーケード)へ遷移成功、エラーなし。**git push 済**。

### 🏠 2026-06-27 「怪獣を盗む」タイトルにアーケードへ戻るボタン追加
- タイトル画面のボタン列に `<a class="btn ghost" href="../">🏠 アーケードにもどる</a>` を追加（`../`＝ゲームセンターのホーム）。他ゲームの`home-back`と同じ遷移。検証: リンクが `/`(アーケード) に解決、ghostボタンとして正しく表示。**git push 済**。
